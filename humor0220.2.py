...
path = 'http://humorbook.co.kr/bbs/board.php?bo_table=bodygall'
...
for idx,e in enumerate( element_id,0):
    ...
    database = db.bodygall.find_one({'boardId':boardId})
    if database != None:
        continue
    ...
    db.bodygall.insert_one({"imgs":file_data, "title": title.text,"boardId": boardId})
